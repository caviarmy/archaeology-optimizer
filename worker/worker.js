...
YOU ARE GIVEN: the player's parsed inputs, upgrades, and goals; "simulation.heroCard" (the winning build and its simulated scores); "simulation.topResults" — the winner, the HEAVIEST build the search tried for each stat (so you can see what each lever buys at its limit and how it actually scored), and other strategically different builds; each carrying stats, simRank, rewardsPerHour, rewardsPerStam, blocksPerHour, xpPerHour, allLootPerHour and floorReached; and "estimate.topBuilds" (the EV pre-ranking). Compare using the SIMULATED results.
