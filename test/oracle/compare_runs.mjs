...
for (const name of Object.keys(theirs)) {
  ...
  console.log(`  ${ok ? "ok  " : "FAIL"} ${name.padEnd(8)} floor theirs=${t.floor.toFixed(2)} ours=${(o.floor ?? NaN).toFixed(2)} (${(fdiff * 100).toFixed(1)}%)  blocks theirs=${t.blocks.toFixed(1)} ours=${(o.blocks ?? NaN).toFixed(1)} (${bdiff}, informational)`);
}
